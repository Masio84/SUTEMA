...
export default function AppLayout({ children, title, subtitle }: { children: React.ReactNode, title: string, subtitle?: string }) {
    ...
    return (
        <div className="flex h-screen bg-[#fafafa] dark:bg-zinc-950 overflow-hidden font-sans">
            {/* Sidebar */}
            <motion.aside
                        ...
                        <nav className="space-y-2">
                            {navItems.map((item) => {
                                ...
                                return (
                                    ...
                                    >
                                        <div className={`transition-transform duration-300 group-hover:scale-110 ${isActive ? 'text-primary' : 'text-zinc-400 group-hover:text-zinc-600 dark:group-hover:text-zinc-300'
                                            }`}>
                                            {item.icon}
                                        </div>
                                        {item.label}
            ...
            <div className="flex-1 flex flex-col min-w-0">
                <header className="h-20 border-b border-zinc-200 dark:border-zinc-800 bg-white/70 dark:bg-black/50 backdrop-blur-xl sticky top-0 z-30 flex items-center justify-between px-8">
                    ...
                    >
                        ...
                        <h1 className="text-xl font-extrabold tracking-tight text-zinc-900 dark:text-zinc-50">{subtitle || title}</h1>
                    </motion.div>
